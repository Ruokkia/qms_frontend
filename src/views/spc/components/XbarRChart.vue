...
function render() {
  ...
  const rSeries = r.map((v) => ({
    value: v,
    itemStyle: {
      color: v == null ? '#5B7A99' : (ru != null && v > ru) || (rl != null && v < rl) ? '#B84B3E' : '#3E7A4E',
    },
  }))
  ...
  chart.setOption({
    ...
    legend: { data: ['均值 X', '极差 R'], top: 4, right: 12, itemWidth: 16, itemHeight: 10, textStyle: { color: '#5B7A99', fontSize: 11 } },
    grid: [
    ...
    yAxis: [
      ...
      { type: 'value', gridIndex: 1, scale: true, name: `极差 R（${unit}）`, nameTextStyle: { color: '#5B7A99', fontSize: 10 }, splitLine: { lineStyle: { color: '#F4F1ED' } }, axisLabel: { color: '#8C9BA8' } },
    ],
    series: [
      {
        name: '均值 X',
      ...
      },
      {
        ...
        markLine: limitLine(ru, rc, rl, '#B84B3E'),
      },
    ],
  }, true)
}
