  ...
  const tip = (params: any) => {
    ...
    if (pt.samples && pt.samples.length > 0) {
      ...
      pt.samples.forEach((sv: any, si: number) => {
        s += `${sv != null ? Number(sv.sampleValue).toFixed(3) : '—'}`
        if (si < pt.samples.length - 1) s += ', '
      })
      s += '<br/>'
    }
    if (pt.sampleTime) s += `录入时间：<b>${pt.sampleTime}</b>`
    return s
  }
...
async function load() {
  ...
  // 图表数据 + 能力指数并行获取
  await Promise.all([
    store.fetchChartDataXbarR(props.paramId, props.itemType, props.itemCode, props.batchNo),
